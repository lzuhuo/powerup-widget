...
Panel {
  id: root
  moduleName: "powerup.power"
  ipcTarget: "powerup.power"
  ...
  function refresh() {
    ...
    if (!sensorProc.running) sensorProc.running = true
  }
  ...
  function updateSensors(raw) {
    ...
    if (Object.keys(next).length > 0) sensorInfo = next
  }
  ...
  Process {
    id: sensorProc
    command: ["sensors", "-j"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateSensors(text) }
  }
        ...
        Column {
          ...
          Row {
            ...
            MetricCard {
              width: (parent.width - parent.spacing * 2) / 3
              label: "TEMP"
              value: root.sensorInfo.temperature || "N/A"
              accent: "#fb923c"
            }
            MetricCard {
              width: (parent.width - parent.spacing * 2) / 3
              label: "FAN SPEED"
              value: root.sensorInfo.fan || "N/A"
              accent: "#38bdf8"
            }
            MetricCard {
              width: (parent.width - parent.spacing * 2) / 3
  ...
  component MetricCard: Rectangle {
    property string label: ""
    property string value: "—"
    property color accent: root.bar.foreground
    ...
    Column {
      ...
      Text {
        ...
        font.bold: true
        elide: Text.ElideRight
        width: parent.parent.width - Style.space(16)
      }
    }
  }
}
